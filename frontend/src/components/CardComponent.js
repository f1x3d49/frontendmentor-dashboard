...
const CardComponent = ({ title }) => {
  return (
    <section className="w-full h-auto bg-dblue rounded-xl font-rubik">
      <div className="bg-lred rounded-t-xl py-6 overflow-hidden relative">
        <img src={work} alt="work" className="absolute -top-4 right-4" />
      </div>
      {/* Content */}
      <div className="flex items-center justify-between p-4 rounded-xl relative -mt-[-0.32rem]">
        ...
        <div className="flex flex-col gap-6">
          <img
            src={dots}
            alt="dots"
            className="self-end hover:cursor-pointer hover:fill-white"
          />
